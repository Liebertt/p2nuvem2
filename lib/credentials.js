...
function getGoogleCredentials() {
  ...
  // Helper para limpar e normalizar a chave privada para o OpenSSL
  ...
  try {
    ...
    if (process.env.GOOGLE_CREDENTIALS_JSON) {
      ...
      console.log(`[DEBUG] GOOGLE_CREDENTIALS_JSON detectada. Tamanho da string: ${trimmed.length} caracteres.`);
      console.log(`[DEBUG] Início da string env: "${trimmed.substring(0, 20)}..."`);
      console.log(`[DEBUG] Fim da string env: "...${trimmed.substring(trimmed.length - 20)}"`);
    ...
    if (credentials && credentials.private_key) {
      ...
      console.log(`[DEBUG] Chave privada processada. Tamanho original: ${originalLength}, após limpeza: ${credentials.private_key.length}`);
      console.log(`[DEBUG] Início da chave: "${credentials.private_key.substring(0, 40)}..."`);
      console.log(`[DEBUG] Fim da chave: "...${credentials.private_key.substring(credentials.private_key.length - 40)}"`);
    } else {
      console.log("[WARNING] Nenhuma chave privada encontrada no objeto de credenciais.");
    }
